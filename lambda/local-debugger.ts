...
const skillInvoker = require(getAndValidateSkillInvokerFile().toString());
...
localDebugger.listen(portNumber, host, () => {
  console.log(
    `Starting server on port: ${
      (localDebugger.address() as net.AddressInfo)!.port
    }.`,
  );
});
...
localDebugger.on("connection", (socket) => {
  ...
  socket.on("data", (data) => {
    const body = JSON.parse(data.toString().split(httpBodyDelimiter).pop()!);
    console.log(`Request envelope: ${JSON.stringify(body)}`);
    skillInvoker[lambdaHandlerName](
      body,
      null,
      (_invokeErr: any, response: any) => {
        response = JSON.stringify(response);
        console.log(`Response envelope: ${response}`);
        socket.write(
          `HTTP/1.1 200 OK${httpHeaderDelimiter}Content-Type: application/json;charset=UTF-8${httpHeaderDelimiter}Content-Length: ${response.length}${httpBodyDelimiter}${response}`,
        );
      },
    );
  });
...
function getAndValidateSkillInvokerFile() {
  ...
  if (!fs.existsSync(fileNameArgument.toString())) {
...
function getArgument(argumentName: string, defaultValue?: string | number) {
