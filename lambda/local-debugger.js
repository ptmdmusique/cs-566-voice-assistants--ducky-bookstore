...
const localDebugger = net_1.default.createServer();
...
localDebugger.on("connection", (socket) => {
    ...
    socket.on("data", (data) => {
        ...
        skillInvoker[lambdaHandlerName](body, null, (_invokeErr, response) => {
            ...
            socket.write(`HTTP/1.1 200 OK${httpHeaderDelimiter}Content-Type: application/json;charset=UTF-8${httpHeaderDelimiter}Content-Length: ${response.length}${httpBodyDelimiter}${response}`);
        });
    });
});
/**
...
function getAndValidateSkillInvokerFile() {
    ...
    if (!fs_1.default.existsSync(fileNameArgument.toString())) {
